...
def test_get_all_rate_response_is_404_because_no_data_from_database(client):
    response = client.get("/api/v1/rate")
...
def test_create_rate_with_multi_json_response_is_400(client, fake_multi_rate):
    response = client.post("/api/v1/rate", json=fake_multi_rate)
    assert response.status_code == 400
        

def test_create_rate_response_is_201(client, fake_rate):
    response = client.post("/api/v1/rate", json=fake_rate)
